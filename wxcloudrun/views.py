...
def upload_pdf_v1():
    ...
    try:
        ...
        messagesWenda = [
            ...
            {"role": "user",
             "content": "你是一个专业老师，请针对" + filename + ".pdf" + "的全部内容，提供10道文档中重要知识点相关的问答题，返回的格式要求：list的json字符串，其中list里面包含map，每个map包含这些属性：question（问题）、fenxi（答案分析解释、知识点复述）、source（答案来源，具体到哪一章哪一节或目录标题，不包含pdf文件名）"}
        ]
