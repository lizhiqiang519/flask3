...
def api_pdf_v1():
    ...
    try:
        ...
        # 注意，之前 retrieve_content api 在最新版本标记了 warning, 可以用下面这行代替
        # 如果是旧版本，可以用 retrieve_content
        app.logger.info("暗面返回的文件ID %s",file_object.id)
        file_content = client.files.content(file_id=file_object.id).text
...
@app.route('/api/pdf/file_id_v2', methods=['POST'])
def upload_pdf_file_id_v2():
    # 解析请求数据
    data = request.get_json()
    downloadURL = data.get('downloadURL')
    pdfName = data.get('pdfName')
    ...
    try:
        ...
        file_content = client.files.content(file_id=api_file_id).text
        ...
        messagesZongJie = [
            ...
            {"role": "user",
             "content": "你是一个老师，请认真阅读" + "全部内容，最终返回的是json的字符串（用```json和```包围起来）。先从专业的角度总结该文档的内容（返回字段是zongjie），然后从专业的角度分点详细总结出各个完整的知识点（返回的字段是fendian,fendian是一个list结构，里面是map，map包含的key有title（目录标题，含序号）、content（该目录的内容的总结））"}
        ]
