...
def upload_pdf():
    ...
    try:
        ...
        messages1 = [
            ...
            {"role": "user", "content": "充分阅读"+ filename +".pdf," +"针对pdf上部分内容，提供8道书中重要知识点相关的选择题，返回的格式要求：list的json字符串格式，list里面包含map，每个map包含这些属性：question（问题）、option_a(选项A)、option_b(选项B)、option_c(选项C)、option_d(选项D)、answer（答案，单选A或B或C或D）、fenxi（答案分析解释、知识点复述）、source（答案来源，具体到哪一章哪一节）"
                                    },
        ]
